...
- `run <target> [args...]`：解析并准备依赖，然后 **exec 为 java**（进程即 java，无 jstart 父子等待）；`--port=8080` 等参数原样传给应用，`-D`/`-X` 开头参数交给 JVM。
- `resolve <target>`：下载缺失依赖到本地仓库（默认 `~/.m2/repository`），成功输出应用绝对路径。
